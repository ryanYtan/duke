...
For duke.ui.Duke, there are two events that we want to respond to, namely the user pressing `Enter` in the `TextField` and left-clicking the `Button`. These are the `onAction` event for the `TextField` and the `onMouseClicked` event for the `Button`.

For now, let’s have the application add a new `Label` with the text from the `TextField`. Update the `duke.ui.Main` class as follows. You'll need to add an `import javafx.scene.control.Label;` too.
```java
@Override
...
Let’s create our custom control `duke.ui.DialogBox`:
```java
...
public class duke.ui.DialogBox extends HBox {
    ...
    public duke.ui.DialogBox(Label l, ImageView iv) {
        text = l;
        displayPicture = iv;
...
We use the code in our main class just like any other control. Here are the steps to update the code to use the custom control in `duke.ui.Main.java`.
...
For this tutorial, we have two images `DaUser.png` and `DaDuke.png` to represent the user avatar and duke.ui.Duke's avatar respectively but you can use any image you want.
...
private void handleUserInput() {
    ...
    dialogContainer.getChildren().addAll(
            new duke.ui.DialogBox(userText, new ImageView(user)),
            new duke.ui.DialogBox(dukeText, new ImageView(duke))
    );
    userInput.clear();
}
...
private String getResponse(String input) {
    return "duke.ui.Duke heard: " + input;
}
```
...
## Iteration 3 – Adding custom behavior to duke.ui.DialogBox

One additional benefit of defining a custom control is that we can add behavior specific to our `duke.ui.DialogBox`. Let’s add a method to flip a dialog box such that the image on the left to differentiate between user input and duke.ui.Duke’s output.
...
public static duke.ui.DialogBox getUserDialog(Label l, ImageView iv) {
    return new duke.ui.DialogBox(l, iv);
}

public static duke.ui.DialogBox getDukeDialog(Label l, ImageView iv) {
    var db = new duke.ui.DialogBox(l, iv);
    db.flip();
    return db;
}
```
...
Now, we can go back to the `duke.ui.Main` class and change the event handler to use our new `duke.ui.DialogBox`. 
...
Congratulations! 
You have successfully implemented a fully functional GUI for duke.ui.Duke!  
...
1. While the GUI looks similar to the mockup, there are still parts that need to be refined. Try your hand at some of these tasks:
   * Add padding between each duke.ui.DialogBox
